...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  late String categoryName; //Using this variable name we will save our Category Name in our Cloud Firestore (Firestore Database)
  ...
  _uploadImageToStorage(dynamic image) async{
    Reference ref = _firebaseStorage.ref().child('categories').child(fileName!); //Here what we are doing is we are creating a file named 'categories' in our Firebase Storage and also uploading the image with its 'fileName' which is the name of our File (Image) in our Local System.
  ...
  uploadToFirestore() async {
    if(_formKey.currentState!.validate()) { //If our form to store category name is validated and an image is selected then only we will perform the uploading action to Firestore.
      if(_image!=null) {
        EasyLoading.show(); //Starting our Easy Loading
        String imageUrl = await _uploadImageToStorage(_image);
        await _firestore.collection('categories').doc(fileName).set({ //Creating the collections in Firestore like we did previously manually in the beginning
          'categoryName': categoryName,  //Creating the fields which we also did previously manually in the beginning
    ...
    } else { //This part means there has been some Error in our Form Validation
      EasyLoading.dismiss();
    }
  }
        ...
        children: [
          ...
          Row( //To place items horizontally
            children: [
              ...
              //Box to Type Category Name. It is a Form Field so that we can directly accept and retrieve the input given.
              SizedBox(
                width: 200, //Width of our Box
                child: TextFormField(
                  onChanged: (value) {
                    categoryName = value; //Assigning the value which our user has typed to 'categoryName' which will store it in our Firestore Database.
                  },
                  validator: (value) {
                    if(value!.isEmpty) {
                      return 'Please Enter the Category Name'; //If our Form is Empty show this message
                    }
                    else {
                      return null;
                    }
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Type the Category Name',
                  ),
                ),
              ),
          ...
          //Displaying our present categories stored in our Firebase
          CategoryListWidget(),
